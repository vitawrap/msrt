...
namespace browser {
    ...
    void ScriptHost::installLibCanvas() {
        ...
        JSValue ctor = JS_NewCFunction2(ctx, [](JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) -> JSValue {
            /** TODO: Use base constructor CFunctions for subclasses even if this currently works! */
            JSTempVal proto = getClassProtoOrThrow(ctx, new_target, classId_HTMLCanvas, "HTMLCanvasElement");
            JSValue obj = JS_NewObjectProtoClass(ctx, proto, classId_HTMLCanvas);
            if (JS_IsException(obj))
                throw ScriptEngineException("HTMLCanvasElement: Failed to construct new C instance!");
            JS_SetOpaque(obj, new ScriptProxy<HTMLCanvasElement>(ctx));
            return obj;
